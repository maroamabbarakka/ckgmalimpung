...
function Pos3() {
  ...
  useEffect(() => {
    const q = query(collection(db, "visits"), where("status_antrian", "in", [STATUS_MAPPING.POS3, 'Menunggu Pos 3', 'Antri Pos 3', 'Antre Pos 3', 'POS 3', 'Pos 3']));
  ...
  const handlePanggil = async (item) => {
    try {
      ...
      setPasienAktif(item); setFormData(sanitizeFormDataForSchema(activeSchema, item.pos3 || {})); window.scrollTo({ top: 0, behavior: 'smooth' });
      try { await addDoc(collection(db, "panggilan_tv"), { pos: "POS 3", identitas_layar: item.nomor_antrian, teks_suara: `Nomor antrean... ${item.nomor_antrian.replace(/-/g, ' ')}... Silakan menuju Pos Tiga.`, waktu: serverTimestamp() }); } catch (e) { console.warn("Gagal membuat panggilan TV Pos 3:", e); }
    } catch (e) {
      alert("⚠️ " + e.message);
    }
  };
  ...
  const handleSimpanData = async (e) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "visits", pasienAktif.id), { status_antrian: STATUS_MAPPING.POS4, petugas_pos3: sessionStorage.getItem('namaPegawai') || 'Sistem', pos3: sanitizedFormData, pos3_question_map: buildQuestionMap(activeSchema), petugas_aktif: null });
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto mobile-safe-page px-2 md:px-0 font-sans">
      {!pasienAktif ? (
        <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {antrian.map((item) => (
                <div key={item.id} onClick={() => handlePanggil(item)} className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col items-center justify-center cursor-pointer hover:border-[#e11d48] group shadow-sm">
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-100 overflow-hidden">
        <div className="bg-[#e11d48] p-6 text-white flex justify-between items-center">
        ...
        <form onSubmit={handleSimpanData} className="p-4 md:p-6 bg-[#f8fafc] mobile-safe-page">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-6 sticky mobile-safe-submit z-40">
              ...
              <button type="submit" disabled={loading} className="w-full min-h-[60px] bg-[#e11d48] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-sm uppercase disabled:opacity-50">
                  {loading ? 'MENYIMPAN...' : 'Simpan & Ke Pos 4 ›'}
              </button>
            </div>
        </form>
      </div>
