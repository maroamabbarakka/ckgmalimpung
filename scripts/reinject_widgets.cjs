...
const anchor = "const handleLogout = () => { sessionStorage.removeItem('isAuthenticated'); navigate('/login'); };\n";
const index = code.indexOf(anchor);

if (index !== -1) {
    code = code.substring(0, index + anchor.length) + missingWidgets + code.substring(index + anchor.length);
