...
// Use regex matching to be completely whitespace agnostic!
const tOldRegex = /<div className="flex flex-wrap gap-2 mb-4">[\s\S]*?<textarea[\s\S]*?required[\s\S]*?\/>/m;

if (tOldRegex.test(code)) {
    fs.writeFileSync('Pos7.jsx', code.replace(tOldRegex, tNew));
    console.log('Replaced using Regex!');
} else {
    console.log('Target block not found using Regex.');
}
